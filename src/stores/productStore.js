...
export const useProductStore = defineStore('product', {
  ...
  getters: {
    ...
    getProductRatings: (state) => (productId) => state.productRatings[productId] || [],
    ...
    getProductRatingsCount: (state) => (productId) => state.productRatings[productId]?.length || 0,
  ...
  actions: {
    ...
    // --- OTHER ACTIONS (Ratings, Invoices, etc.) ---
    // These actions do not need changes.
    ...
    async fetchPurchaseInvoices({ page = 1, search = '' } = {}) {
      ...
      try {
        ...
        // Store complete API data including images
        this.purchaseInvoices = invoiceList.map(invoice => ({
          ...
          items: invoice.items.map(item => ({
            ...
            images: item.images || [] // ✅ NEW: Store images array
          }))
        }));
